...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/podcastindex_search_podcasts \
  -H 'Content-Type: application/json' \
  -d '{"query":"true crime","max":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/podcastindex_search_podcasts`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "podcastindex": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-podcastindex"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-podcastindex
```
